...
function createDashboardServer(bot) {
  const secret = process.env.WEB_SESSION_SECRET || process.env.BOT_TOKEN || crypto.randomBytes(32).toString('hex');
  const auth = signer(secret);
  const adminToken = String(process.env.WEB_ADMIN_TOKEN || '');
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/config') {
        return json(res, 200, {
          ...
          adminEnabled: Boolean(adminToken), auth: session ? { role: session.role, userId: session.userId, username: session.username } : null
        });
      }
      ...
      if (req.method === 'POST' && url.pathname === '/api/auth/telegram/verify') {
        ...
        const db = await catalog.loadUsers();
        const user = db.users?.[userId];
        setSession(res, auth, { role: 'user', userId, username: user?.username || user?.first_name || userId });
        return json(res, 200, { ok: true });
      }
      if (req.method === 'POST' && url.pathname === '/api/auth/admin') {
        const input = await body(req);
        const ip = req.socket.remoteAddress || 'unknown';
        if (!rateLimit(`admin:${ip}`, 10, 15 * 60 * 1000)) return fail(res, 429, 'Terlalu banyak percobaan login.');
        if (!adminToken) return fail(res, 503, 'Login admin web belum dikonfigurasi.');
        const supplied = Buffer.from(String(input.token || ''));
        const expected = Buffer.from(adminToken);
        if (supplied.length !== expected.length || !crypto.timingSafeEqual(supplied, expected)) return fail(res, 401, 'Token admin salah.');
        setSession(res, auth, { role: 'admin', username: 'Admin' });
        return json(res, 200, { ok: true });
      }
...
function startDashboard(bot) {
  if (String(process.env.WEB_ENABLED || 'true').toLowerCase() === 'false') return null;
