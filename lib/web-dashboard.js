...
function safeTx(tx, admin = false) {
  const item = {
    ...
    accounts: Array.isArray(tx.delivered_accounts) ? tx.delivered_accounts : []
  };
  if (admin) item.userId = tx.user_id;
  if (lower(tx.status) === 'pending' && process.env.QR_STRING) {
    item.qrUrl = new QRIS(item.amount, process.env.QR_STRING).toURL();
  }
  if (tx.expires_at) item.expiresAt = Number(tx.expires_at);
  return item;
}
...
async function getBotUsername(bot) {
  const configured = String(process.env.BOT_USERNAME || '').replace(/^@/, '').trim();
  if (configured) return configured;
  if (!bot?.telegram || typeof bot.telegram.getMe !== 'function') {
    throw Object.assign(new Error('Bot Telegram belum tersedia di proses web.'), { statusCode: 503 });
  }
  if (!botUsernamePromise) {
    botUsernamePromise = bot.telegram.getMe().then((me) => {
      const username = String(me?.username || '').replace(/^@/, '').trim();
      if (!username) throw Object.assign(new Error('Bot Telegram tidak memiliki username.'), { statusCode: 503 });
      return username;
    }).catch((error) => {
      botUsernamePromise = null;
      throw Object.assign(new Error('Bot Telegram tidak bisa dihubungi. Pastikan BOT_TOKEN aktif dan koneksi Telegram tersedia.'), { statusCode: 503, cause: error });
    });
  }
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/api/orders') {
        ...
        return json(res, 201, { ...result, expiresAt: result.expiresAt || (result.id ? (await txDb.findById(result.id))?.expires_at : null) });
      }
      const orderStatusMatch = url.pathname.match(/^\/api\/orders\/([^/]+)\/status$/);
      if (req.method === 'GET' && orderStatusMatch) {
        if (!session || !['user', 'admin'].includes(session.role)) return fail(res, 401, 'Silakan login terlebih dahulu.');
        const transaction = await txDb.findById(Number(decodeURIComponent(orderStatusMatch[1])));
        if (!transaction || String(transaction.user_id) !== String(session.userId) || transaction.source !== 'web') return fail(res, 404, 'Order tidak ditemukan.');
        return json(res, 200, safeTx(transaction));
      }
