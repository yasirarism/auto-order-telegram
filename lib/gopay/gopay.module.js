...
class gopay {
  ...
  constructor(opts) {
    ...
    const envTimeout = Number(process.env.GOBIZ_REQUEST_TIMEOUT_MS);
    this.timeoutMs = Number.isFinite(envTimeout) && envTimeout > 0 ? envTimeout : (opts?.timeoutMs ?? 15000);
  }
  ...
  async getTokensByPassword(email, password) {
    const resLogin = await this.fetchWithTimeout(`${BASE_URL}/goid/login/request`, {
      method: 'POST',
    ...
    const resToken = await this.fetchWithTimeout(`${BASE_URL}/goid/token`, {
      method: 'POST',
  ...
  async searchJournals(access_token, payload) {
    const res = await this.fetchWithTimeout(`${BASE_URL}/journals/search`, {
      method: 'POST',
  ...
  async getMe(access_token) {
    const res = await this.fetchWithTimeout(`${BASE_URL}/v1/users/me`, {
      method: 'GET',
  ...
  async fetchWithTimeout(url, init) {
    ...
    try {
      ...
      if (err && err.name === 'AbortError') {
        logger.error(`${traceKey}request timeout after ${this.timeoutMs}ms for ${url}`);
      }
      throw err;
    } finally {
