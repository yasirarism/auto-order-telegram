...
class gopay {
  ...
  async getTokensByPassword(email, password) {
    logger.info(`${traceKey}login request start`);
    ...
    logger.info(`${traceKey}token request start (password)`);
  ...
  async getTokensByRefreshToken(refresh_token) {
    logger.info(`${traceKey}token request start (refresh)`);
    const resToken = await fetchWithTimeout(`${BASE_URL}/goid/token`, {
      method: 'POST',
      headers: this.buildHeaders('token'),
      body: JSON.stringify({
  ...
  async searchJournals(access_token, payload) {
    logger.info(`${traceKey}journals search start`);
  ...
  async getMe(access_token) {
    logger.info(`${traceKey}get me start`);
