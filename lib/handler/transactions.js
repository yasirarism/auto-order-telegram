...
const bannerPath = path.resolve(__dirname, '../../assets/expired.jpg')
const { sendPaymentAnnouncement } = require('../../utils/paymentCard');
const CHANNEL_TARGET = process.env.CHANNEL_TARGET;
const STORE_NAME = process.env.STORE_NAME || 'SEN PRO';
...
if (r.status === 'paid' && CHANNEL_TARGET && !r.posted_to_channel) {
  try {
    ...
    logger.error(traceKey, 'error kirim ke channel:', err.message);
      ...
      if (delivered === 0) {
        ...
        try {
          ...
          const soldBannerPath = require('path').join(__dirname, '../../assets/sold.jpg');
          ...
          const cap = [
            ...
            'Copy pesan ini dan kirim ke admin @sphynixstore untuk refund / kirim manual.',
          ].join('\n');
  ...
  '╰───────────────────────╯'
      ...
      if (shortage > 0) {
        ...
        orderCaption = [
          ...
          'Silakan copy pesan ini dan kirim ke admin @sphynixstore.'
        ].join('\n');
      } else {
      ...
      await tx.update(r.id, { ...r, sent_account: true });
      if (r?.user_id && r?.message_id) {
        await deleteMessageSafe(bot, r.user_id, r.message_id);
      }
      ...
      if (v && v.snk) {
        ...
        const snkText = `<b>📋 Syarat & Ketentuan</b>\n\n${esc(String(v.snk || ''))}`;
...
async function getEffectiveStock(product, variantName) {
  ...
  if (fs.existsSync(stockPath)) {
    try {
      stokList = JSON.parse(fs.readFileSync(stockPath, 'utf8')) || [];
    } catch {
      stokList = [];
    }
  }
