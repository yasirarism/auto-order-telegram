...
const { ce, applyCustomEmoji } = require('../../utils/customEmoji');
...
async function sendMessageSafe(bot, chatId, text, extra = {}) {
  try {
    ...
    if (typeof bot.reply === 'function') return await bot.reply(formattedText, payload);
    if (bot.telegram && typeof bot.telegram.sendMessage === 'function') return await bot.telegram.sendMessage(chatId, formattedText, payload);
    if (typeof bot.sendMessage === 'function') return await bot.sendMessage(chatId, formattedText, payload);
  } catch (e) {
...
async function sendPhotoSafe(bot, chatId, photoInput, caption = '', extra = {}) {
  try {
    ...
    const payload = { caption: formattedCaption || '', parse_mode: 'HTML', ...extra };
...
function successCapt({ txRef, orderRef, buyer, paymentMethod, orderIdDisplay, orderDate, orderTotal, orderStatus }) {
  return [
    '╭───────────────────────╮',
    `├ <b>${ce('success', '✅')} PEMBAYARAN BERHASIL</b>`,
    `├ <b>Buyer:</b> ${buyer}`,
